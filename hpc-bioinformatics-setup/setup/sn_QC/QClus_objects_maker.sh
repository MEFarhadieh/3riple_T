############################################################################################
# made QClus objects for all samples
############################################################################################
...
base_idx = fu.index.astype(str).str.strip().str.replace(r"-\d+$", "", regex=True)  # <- fixed .str.strip()
fu_both = pd.concat([
...
PYCODE
done < /mnt/archive/farhadie/farhadie/cellranger_runs/samples.txt
